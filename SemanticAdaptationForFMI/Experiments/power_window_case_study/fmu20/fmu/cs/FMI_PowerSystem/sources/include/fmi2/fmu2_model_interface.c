...
fmi2Status fmi2EnterInitializationMode(fmi2Component c) {
  ...
  if (invalidState(comp, "fmi2EnterInitializationMode", modelInstantiated))
    return fmi2Error;
  printf("PowerSystem first logging message...\n");
  FILTERED_LOG(comp, fmi2OK, LOG_FMI2_CALL, "fmi2EnterInitializationMode...")
  FILTERED_LOG(comp, fmi2OK, LOG_FMI2_CALL, "Testing access to component members...")
  FILTERED_LOG(comp, fmi2OK, LOG_FMI2_CALL, "Tolerance = %g", comp->tolerance)
  
  /* set zero-crossing tolerance */
  FILTERED_LOG(comp, fmi2OK, LOG_FMI2_CALL, "Setting zero crossing tolerance...")
  setZCtol(comp->tolerance);
  FILTERED_LOG(comp, fmi2OK, LOG_FMI2_CALL, "Setting start values...")
  setStartValues(comp);
  FILTERED_LOG(comp, fmi2OK, LOG_FMI2_CALL, "Copying start values to init values...")
  copyStartValuestoInitValues(comp->fmuData);

  FILTERED_LOG(comp, fmi2OK, LOG_FMI2_CALL, "MMC_TRY_INTERNAL...")
  /* try */
  MMC_TRY_INTERNAL(simulationJumpBuffer)
  
    FILTERED_LOG(comp, fmi2OK, LOG_FMI2_CALL, "initialization...")
    ...
    else
    {
      ...
      FILTERED_LOG(comp, fmi2OK, LOG_FMI2_CALL, "initSample...")
      ...
      FILTERED_LOG(comp, fmi2OK, LOG_FMI2_CALL, "initDelay...")
	  initDelay(comp->fmuData, comp->fmuData->localData[0]->timeValue);
#endif
      /* due to an event overwrite old values */
      FILTERED_LOG(comp, fmi2OK, LOG_FMI2_CALL, "overwriteOldSimulationData...")
	  overwriteOldSimulationData(comp->fmuData);

      FILTERED_LOG(comp, fmi2OK, LOG_FMI2_CALL, "comp->eventInfo.terminateSimulation...")
	  comp->eventInfo.terminateSimulation = fmi2False;
      FILTERED_LOG(comp, fmi2OK, LOG_FMI2_CALL, "comp->eventInfo.valuesOfContinuousStatesChanged...")
	  comp->eventInfo.valuesOfContinuousStatesChanged = fmi2True;
	  
	  FILTERED_LOG(comp, fmi2OK, LOG_FMI2_CALL, "Getting next sample event...")
      ...
      if (nextSampleEvent == -1) {
        FILTERED_LOG(comp, fmi2OK, LOG_FMI2_CALL, "No next sample event...")
		comp->eventInfo.nextEventTimeDefined = fmi2False;
      } else {
		FILTERED_LOG(comp, fmi2OK, LOG_FMI2_CALL, "Next sample event found...")
        comp->eventInfo.nextEventTimeDefined = fmi2True;
        comp->eventInfo.nextEventTime = nextSampleEvent;
        fmi2EventUpdate(comp, &(comp->eventInfo));
      }
	  
	  FILTERED_LOG(comp, fmi2OK, LOG_FMI2_CALL, "Setting comp->state to modelInitializationMode...")
