...
class CppGenerator {
	...
	def void compile(SemanticAdaptation adaptation, String resolvedFolderURI) {
		for (Adaptation adap : adaptation.elements.filter(Adaptation)) {
			 ...
			 */
			var ArrayList<ModelDescription> mds = newArrayList();
			...
			for (fmu : adap.inner.eAllContents.toList.filter(InnerFMU)) {
				...
				var md = new ModelDescription(fmu.name, fmuFile);
				mds.add(md);
			...
			val outRuleResult = compileInOutRuleBlocks(IORuleType.Output, outRules, adapClassName, adapInteralRefName,
				mappedScalarVariables, SASVs, params, if(inVars !== null) inVars.value else null,
				if(outVars !== null) outVars.value else null, if(crtlVars !== null) crtlVars.value else null,
				outPortsWithSrcDep);
  	...
  	def getFMUFile(String fmuUnresolvedPath, String resolvedFolderURI) {
		...
		val fmuCompleteURI = URI.createFileURI(resolvedFolderURI + File.separatorChar +
			fmuUnresolvedPath.replace('\"', ''))
	...
	def InOutRulesBlockResult compileControlRuleBlock(ControlRuleBlock crtlRuleBlock, String adaptationClassName,
		...
		val visitor = new ControlConditionSwitch(
			adaptationClassName,
			adaptationName,
			mSVars,
			SASVs,
	...
	def InOutRulesBlockResult compileInOutRuleBlocks(
		...
		if (!visitor.functionSignatures.empty || !portsWithSrcDep.empty) {
			...
			for (var int i = 0; i < portsWithSrcDep.length; i++) {
				...
				val funcSigBody = visitor.createFunctionSignature('''body''', "void", newCount, depFunctionSignatures)
				val funcSigFlush = visitor.createFunctionSignature('''flush''', "void", newCount, depFunctionSignatures)
		...
		for (inport : inports) {
			...
			if (inport.targetdependency !== null) {
				saSV.type = mSVs.get(inport.targetdependency.owner.name).get(inport.targetdependency.port.name).
					mappedSv.type;
				saSV.variability = mSVs.get(inport.targetdependency.owner.name).get(inport.targetdependency.port.name).
					mappedSv.variability;
			}
		}
