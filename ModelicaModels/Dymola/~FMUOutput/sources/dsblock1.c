...
#if defined(UNIX) && defined(__GNUC__) && defined(FMI_MODULE_NAME) 
...
#if DymolaUninterruptedTimers_ && defined(_MSC_VER) && defined(_M_IX86)
		__asm {
			sti
		}
...
1
	...
	1
      ...
      static Dymola_bool sectioncondition = true;
...
const char* dymosimFMIPath() {
	...
	extraPath[0] = 0;
	GetModuleFileNameA(hMySelf, extraPath, sizeof(extraPath)/sizeof(*extraPath));
	return extraPath;
#else
	return 0;
#endif
...
DYMOLA_STATIC const char* dymosimResources(void) {
    ...
    extraPath[0] = 0;
		if (GetModuleFileNameA(hMySelf, extraPath, sizeof(extraPath)/sizeof(*extraPath))!=0) {
...
#elif defined(__GNUC__)
    {
      ...
      if (dladdr(__builtin_return_address(0), &dli) != 0 && dli.dli_fname != NULL) {
        strcpy(extraPath, dli.dli_fname);
      } else {
       ...
       /* neither Windows nor gcc, neglect */
	{
		const char* separator = "/";
			...
			extraPath[sizeof(extraPath)/sizeof(*extraPath)-1]=0; /* /Make sure it is NUL-terminated */
			last=strrchr(extraPath,'\\');
			if (last==0) last=strrchr(extraPath,'/');
			if (last && twice) {
				last[0]=0;
				last=strrchr(extraPath,'\\');
				if (last==0) last=strrchr(extraPath,'/');
			...
			if (last) last[1]=0;
			if (twice) {
				strcat(extraPath,"resources");
				strcat(extraPath,separator);
			} else {
				strcat(extraPath,"DymosimResources");
				strcat(extraPath,separator);
			}	
			dymosimResourceLocation=extraPath;
		}
		first=0;
	}
...
static void handleevent4funcMinor(struct DYNInstanceData*did_,const char*rele,const char*sube,int index, int PrintEvent2,int ltz,int invres) {
	handleevent4(rele,sube,&QL_[index], &Qp_[index-1],&Qn_[index-1],EPS_*Qscaled_[index-1],QRel_[index-1],Init_, PrintEvent2,&AnyEvent_,ltz,invres,Qenable_[index],LargeEPS_*Qscaled_[index-1],Iter>SlowIter_);
}
static void handleevent4func(struct DYNInstanceData*did_,const char*rele,const char*sube,int index, int PrintEvent2,int ltz,int invres) {
	handleevent5(rele,sube,&QL_[index], &Qp_[index-1],&Qn_[index-1],EPS_*Qscaled_[index-1],QRel_[index-1],Init_, PrintEvent2,&AnyEvent_,&AnyDEvent_,ltz,invres,Qenable_[index],LargeEPS_*Qscaled_[index-1],Iter>SlowIter_);
}
static Dymola_bool handleevent4Sfunc(struct DYNInstanceData*did_,const char*rele,const char*sube,double*QZV,int index, int incross,double Time,int PrintEvent2,int ltz,int invres) {
	Dymola_bool AE=0;
	Dymola_bool res;
	res=handleevent4S(rele,sube,
		&QL_[index],&Qp_[index-1],&Qn_[index-1],EPS_,QRel_[index-1],&QZV[2*(index)-2],Init_,DYNEvent,incross,PrintEvent2,&AE,ltz,invres,&Qenable_[index],LargeEPS_,Iter>SlowIter_,DYNTime);
