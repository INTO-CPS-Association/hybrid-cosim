...
#define SolveLinearSystemOfEquations(A, b, x, num) \
	...
	*(factoredForParametersP_) = dymolaParametersNr_; \
    ...
    dymli5_(&i3_, &i4_, &(GetMatrix(A,1,1)), &i1_, &i2_, &(GetVector(b,1)), &(GetVector(x,1)),\
      &DYNTime, &DYNEvent, &PrintEvent, A##__DWork, A##__IWork,(A##__FactoredP),varnames_,&Ierror_,0);\
    RealAssign(x,b);\
...
#define MODELICA_RETURNNONNULLATTR /* empty since use of _Ret_z_ in matriop.c yileds:  error C2059: syntax error : '[' */
#else
#define MODELICA_NONNULLATTR
#define MODELICA_RETURNNONNULLATTR
#endif
...
#define _Out_
  ...
  static void ModelicaInternal_mkdir(_In_z_ const char* directoryName);
  static void ModelicaInternal_rmdir(_In_z_ const char* directoryName);
  static int  ModelicaInternal_stat(_In_z_ const char* name);
  static void ModelicaInternal_rename(_In_z_ const char* oldName, _In_z_ const char* newName) ;
  static void ModelicaInternal_removeFile(_In_z_ const char* file);
  static void ModelicaInternal_copyFile(_In_z_ const char* oldFile, _In_z_ const char* newFile);
  static void ModelicaInternal_readDirectory(_In_z_ const char* directory, int nFiles, _Out_ const char* files[]);
  static int  ModelicaInternal_getNumberOfFiles(_In_z_ const char* directory);
  static const char* ModelicaInternal_fullPathName(_In_z_ const char* name);
static const char* ModelicaInternal_temporaryFileName();
  static void ModelicaInternal_print(_In_z_ const char* string, _In_z_ const char* fileName);
  static int  ModelicaInternal_countLines(_In_z_ const char* fileName);
  static void ModelicaInternal_readFile(_In_z_ const char* fileName, _Out_ const char* string[], size_t nLines);
  static const char* ModelicaInternal_readLine(_In_z_ const char* fileName, int lineNumber, _Out_ int* endOfFile);
  static void ModelicaInternal_chdir(_In_z_ const char* directoryName);
  static const char* ModelicaInternal_getcwd(int dummy);
  static void ModelicaInternal_getenv(_In_z_ const char* name, int convertToSlash, _Out_ const char**, _Out_ int* exist);
  static void ModelicaInternal_setenv(_In_z_ const char* name, _In_z_ const char* value, int convertFromSlash);
  ...
  static void ModelicaStreams_closeFile(_In_z_ const char* fileName);
...
/**
 * Internal builtin function used to convert an integer value to an enumeration value (cf. MCP-0022).
