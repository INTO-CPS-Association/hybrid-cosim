  ...
  *icall_ = 0;
  ...
  if (NotInitialized) {
    ...
    }
	{
		...
		getBasicIDymosimStruct()->mFindEvent=(getBasicIDymosimStruct()->mInlineIntegration==1 && getBasicDDymosimStruct()->mDymolaFixedStepSize>0) 
  ...
  if(*ierr_ != 0) {
    ssSetErrorStatus(S, "Error encountered in S-function MEX file. See Message in Matlab window.");
    return;
